...
| Validation how-to (tester) | tutorial (user) |
|---|---|
...
failure, see §4 — the handling is the opposite of a validation how-to's.
...
The validation-how-to rule is *warn the tester, then have them confirm the
defect*. **The
...
- **This is a customer document.** No `Can you do it?` boxes, no defect
  ...
  If those are wanted, the validation `howto` skill is the right one.
